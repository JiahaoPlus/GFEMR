...
if numel(size(I1))>2
    if size(I1,1)==size(I2,1)&&size(I1,2)==size(I2,2)&&size(I1,3)==size(I2,3)
        WhiteInterval = 0*ones(size(I1,1), interval, size(I1,3));
        imagesc(cat(2, I1, WhiteInterval, I2)) ;
        hold on ;
        line([X(FalsePos,1)'; Y(FalsePos,1)'+size(I1,2)+interval], [X(FalsePos,2)' ;  Y(FalsePos,2)'],'linewidth', 1, 'color', 'y') ;
    %     line([X(FalseNeg,1)'; Y(FalseNeg,1)'+size(I1,2)+interval], [X(FalseNeg,2)' ;  Y(FalseNeg,2)'],'linewidth', 1, 'color', 'w') ;
        ...
        I1=padarray(I1,[max(size(I1,1),size(I2,1))-size(I1,1) max(size(I1,2),size(I2,2))-size(I1,2)],'post');
        I2=padarray(I2,[max(size(I1,1),size(I2,1))-size(I2,1) max(size(I1,2),size(I2,2))-size(I2,2)],'post');
        WhiteInterval = 0*ones(size(I1,1), interval, size(I1,3));
        imagesc(cat(2, I1, WhiteInterval, I2)) ;
        hold on ;
        line([X(FalsePos,1)'; Y(FalsePos,1)'+size(I1,2)+interval], [X(FalsePos,2)' ;  Y(FalsePos,2)'],'linewidth', 1, 'color', 'y') ;
    ...
    if size(I1,1)==size(I2,1)&&size(I1,2)==size(I2,2)
        WhiteInterval = 0*ones(size(I1,1), interval);
%         imagesc(cat(2, I1, WhiteInterval, I2)) ;
